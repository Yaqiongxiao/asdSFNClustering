# SNF_Louvain.R
#
# 1) Perform similarity fusion network analysis with 4 ROI variables (frontal and 
# temporal ROI activation) and 6 clinical variables; 
...
SNF_Louvain <- function(dat, ROI_var, clinic_var, n) {
	...
	#estimationResult = estimateNumberOfClustersGivenGraph(W, 2:6)
	
	# spectralClustering
	SpectralClust <- spectralClustering(W, 5)
	...
	#write.csv(snf_matrix[3:6], here(savepath,"snf_matrix.csv"),
	#	  row.names = F, quote=F)
	...
	for (i in 1:dim(W)[1]) {
		...
		top <- tmp_weight[as.numeric(tmp_weight$Weight) > 
				  	quantile(as.numeric(tmp_weight$Weight), 
				  		 prob = 1 - n/100),]
	...
	G1 <- graph_from_adjacency_matrix(gg, mode = "undirected",diag = F,
					  weighted = TRUE) # 
...
#	set.seed(2019)
	...
	W_Clustering_thr$group <- dat$Dx

	return(list(snf_matrix_thr, W_Clustering_thr,SpectralClust,G1,W))
}
